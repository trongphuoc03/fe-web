document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch('assets/data/combos.json')
        ...
        .then(combos => {
            ...
            if (combo) {
                ...
                document.getElementById('combo-location').textContent = combo.location;
                document.getElementById('combo-price').textContent = combo.price.toLocaleString();
            
                document.getElementById('combo-description').textContent = combo.description;
                // Cập nhật giá mỗi người sau khi lấy dữ liệu combo
                comboPricePerPerson = combo.price;
            ...
            if (combo && combo.images && combo.images.length > 0) {
                // Hiển thị ảnh đầu tiên trong carousel
                displayMainImage(combo.images[0]);
                ...
                console.error('No images found for this combo.');
            }
        })
        .catch(error => console.error('Lỗi khi tải dữ liệu combo:', error));
    ...
    function updateTotalPrice() {
        ...
        if (bookingDate && numPeople > 0) {
            const totalPrice = numPeople * comboPricePerPerson;
    ...
    document.getElementById('add-to-order-button').addEventListener('click', function () {
        const order = {
            comboId: comboId, 
            bookingDate: document.getElementById("booking-date").value, // Ngày đặt combo
    ...
    fetch('assets/data/feedback.json')
        .then(response => response.json())
        .then(feedbacks => {
            // Lọc feedback theo ID combo
            const comboFeedbacks = feedbacks.filter(feedback => feedback.combo_id == comboId);
            const feedbackList = document.getElementById('feedback-list');

            comboFeedbacks.forEach(feedback => {
