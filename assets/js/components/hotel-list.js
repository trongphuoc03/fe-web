...
async function getAllHotels() {
  try {
    ...
    for (const hotel of hotels) {
      const feedbackData = await callAPI(
        `https://symfony-9z0y.onrender.com/feedbacks/hotel/${hotel.id}`,
        "GET"
      );
...
function renderPagination(totalItems) {
  ...
  const prevButton = `<a id="prev-btn" href="#" class="flex items-center justify-center px-3 h-8 text-sm font-medium ${
  ...
  const nextButton = `<a id="next-btn" href="#" class="flex items-center justify-center px-3 h-8 text-sm font-medium ${
